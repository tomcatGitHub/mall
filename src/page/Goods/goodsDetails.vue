<!--商品详情-->
<template>
  <div v-loading="loading" element-loading-text="数据加载中...,请稍候" style="min-height: 35vw;">
...
<script>
  ...
  export default {
    ...
    created () { // 生命周期的钩子函数
      let id = this.$route.query.productId // 这种方式是通过query传递路径参数 这种方式传递路径参数时刷新当前页面不会造成路径参数丢失
      // let id = this.$route.params.productId // 这种方式是通过params传递路径参数 这种方式传递路径参数时刷新当前页面会造成路径参数丢失,慎用
      this._productDet(id)
